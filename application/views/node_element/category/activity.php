...
<?php
    ...
    for ($x=0;$x<count($stream);$x++)
    {
        ...
        // format the stream string
            ...
            {
                $strstring.="<a href='/".$stream[$x]['al']."'>".$stream[$x]['an']."</a> ".str_replace('%_COUNT',$count,$actions[$stream[$x]['a']][$plur])." <a href='/".$stream[$x]['nl']."'>".$stream[$x]['nn']."</a>";
            }
            $strstring.="</span>";
        ...
        // viewed or not
            if ($user['user_id']==$node['id']) // signed in user viewing own node
            {
                if (isset($stream[$x]['v'])&&0==$stream[$x]['v'])
                {
                    ...
                    $txclass='strdate strdate notify';
                    $stream[$x]['v']=1;
            ...
            {
                $class='strpanel viewed';
                $txclass='strdate strdate_viewed'; // always show as viewed for everyone else
            }
